...
from files.rnnsimilar import Predictor
...
parser.add_argument('--embedding-file', type=str, default="data/embeddings/glove.840B.300d.txt",
    ...
    for article in data:
        for paragraph in article['paragraphs']:
            ...
            for qa in paragraph['qas']:
                ...
                examples.append((context, qa['question'], qa['label']))


results = {}
for i in tqdm(range(0, len(examples), args.batch_size)):
    predictions = predictor.predict_batch(args,
        examples[i:i + args.batch_size], top_n=args.top_n
    )
    #print ([p for p in predictions[0]])
    for j in range(len(predictions)):
        if qids[i + j] in results:
            results[qids[i + j]].append([(p[0], p[1]) for p in predictions[j]])

        else:
            results[qids[i + j]] = [([(p[0],p[1]) for p in predictions[j]])]
